    ...
    return (
        ...
        <div className="flex justify-between mx-4 my-2">
                <div className="flex justify-between gap-1 items-center">
                    {tags.map((tag, index) => <Link className="hover:underline primarycolored theme-set_url-link mr-2" key={index} href={`/posts/tag/${tag}`} >{tag}</Link>)}
                </div>
                ...
                <Link href={`/users/${userId}`} className="hover:underline primarycolored place-self-center theme-set_url-link">more posts by this user</Link>
            </div>
        </main>
    )
}